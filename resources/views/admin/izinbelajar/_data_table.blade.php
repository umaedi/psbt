        ...
        <tbody>
            @forelse ($table as $key => $tb)
                <tr>
                    ...
                    <td>
                        @if ($tb->status == 'diproses')
                        <span class="badge badge-success">{{ $tb->status }}</span></td>
                        @elseif($tb->status == 'dalam antrian')
                        <span class="badge badge-warning">perlu di proses</span></td>
                        @elseif($tb->status == 'ditolak')
                        <span class="badge badge-danger">{{ $tb->status }}</span></td>
                        @else
                        <span class="badge badge-primary">{{ $tb->status }}</span></td>
                        @endif
                    <td>
                        <a href="/admin/permohonan_izin_belajar/show/{{ $tb->id }}" class="btn btn-sm btn-info">Lihat</a>
                        @if ($tb->status == 'diterima')
                        <a href="{{ route('surat_izin',['folder' => 'izin_belajar', 'filename' => $tb->suratizin]) }}" target="_blank" class="btn btn-sm btn-primary">Unduh surat balasan</a>
                        @endif
                    </td>
                </tr>
            @empty
